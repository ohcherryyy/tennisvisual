...
if __name__ == "__main__":
    with DB(db='matchstat') as db:
        ...
        result=db.fetchall()
        sq="INSERT INTO `matchstat`.`2015ausopenlinks`(`source`,`target`,`label`,`id`,`value`) VALUES(%s,%s,%s,%s,%s)"
        for x in range(0,len(result)):
            if(result[x][2]=="Finals"):
                db.execute(sq,(result[x][1],result[x][0],"决赛",result[x][3],40))
            if(result[x][2]=="Semi-Finals"):
                db.execute(sq,(result[x][1],result[x][0],"半决赛",result[x][3],35))
            if(result[x][2]=="Quarter-Finals"):
                db.execute(sq,(result[x][1],result[x][0],"四分之一决赛",result[x][3],30))
            if(result[x][2]=="Round of 16"):
                db.execute(sq,(result[x][1],result[x][0],"第四轮",result[x][3],25))
            if(result[x][2]=="Round of 32"):
                db.execute(sq,(result[x][1],result[x][0],"第三轮",result[x][3],20))
            if(result[x][2]=="Round of 64"):
                db.execute(sq,(result[x][1],result[x][0],"第二轮",result[x][3],15))
            if(result[x][2]=="Round of 128"):
                db.execute(sq,(result[x][1],result[x][0],"第一轮",result[x][3],10))
